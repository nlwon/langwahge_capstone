...
for epoch in range(180):
    indexes = np.arange((len(resnet_vectors)))
    np.random.shuffle(indexes)
    for batch_count in range(0,len(resnet_vectors)//batch_size):
        batch_indexes = indexes[batch_count*batch_size: batch_count*(batch_size+1)]
        batch = resnet_vectors[batch_indexes]  
        #print(batch)
        prediction = model(batch)
        
        confuser = model(glove[random.choice(list(glove))])
